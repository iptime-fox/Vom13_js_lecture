...
// 객체 타입 : function, array, object(객체 ex.json)
...
// const tag = `<div class="${className}"></div>`; //탬플릿 리터럴
// console.log(tag);
...
// 숫자형 문자는 "+ 의 경우 문자 연산"이 되지만 나머지 연산은 엔진이 숫자로 임의 변경하여 연산한다. 이를 암묵적 형변환이라 한다.
// 암묵적 형변환된 데이터는 숫자형으로 변경된다.
// const d = 3;
// const e = '4';
// console.log(d + e); // 문자임(34)
// console.log(e - d); // 숫자 1이 됨
...
// if (age > 17) {
//   document.write('술먹을 수 있음 적셔!');
// } else {
//   document.write('미자! 술먹을 수 없음');
// }

// 6. undefined
// undefined 는 원시값(Primitive Type)으로, 선언한 후에 값을 할당하지 않은 변수나 값이 주어지지 않은 인수에 자동으로 할당된다.
// undefined는 값이 없으므로 특정 기증이 작동하지 않는 결과를 가져올 수 있다. 하지만 오류는 아니라서 런타임은 진행된다.
var i;
console.log(i);
...
// 하지만 데이터의 타입은 undefined와 다르다.(값은 같지만 타입이 다르다)
